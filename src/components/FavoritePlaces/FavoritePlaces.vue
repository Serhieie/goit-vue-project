...
import { deleteFavoritePlace, updateFavoritePlace } from '../../api/favoritePlaces'
...
const handleSubmit = (formData) => {
  ...
  const { title, description, img, coordinates, id } = formData
  newFormData.append('id', id)
  newFormData.append('title', title)
  newFormData.append('description', description)
  newFormData.append('img', img)
  newFormData.append('coordinates', `[${coordinates}]`)

  updatePlace(newFormData)
}
...
<template>
  <div class="px-6 text-black h-sidebar-height overflow-y-auto">
  ...
  <div class="w-full px-6">
    <IButton class="w-full mt-6" variant="gradient" @click="emit('create')">
      Додати маркер
    </IButton>
  </div>
</template>
