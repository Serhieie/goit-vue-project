...
import { addFavoritePlace, getFavoritePlaces } from '../api/favoritePlaces'
...
const handleAddPlace = async (formData, resetForm) => {
  ...
  newFormData.append('coordinates', `[${mapMarkerLngLat.value}]`)

  await addPlace(newFormData)
  resetForm()
}
...
<template>
  <main class="flex h-screen">
    <div class="relative bg-white h-full w-[400px] shrink-0 overflow-auto">
      <UserInfo />
      <div v-if="isPlacesLoading" class="text-black px-6 text-center">Loading...</div>
      <FavoritePlaces
